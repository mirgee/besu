...
import org.hyperledger.besu.ethereum.trie.MerkleTrie; import org.hyperledger.besu.ethereum.trie.MerkleTrieException;
...
public class BonsaiCachedMerkleTrieLoader implements StorageSubscriber {
    ...
    try {
      final StoredMerklePatriciaTrie<Bytes, Bytes> accountTrie =
          new StoredMerklePatriciaTrie<>(
              (location, hash) -> {
                LOG.info("[PRELOAD ACCOUNT] - " + System.nanoTime() + ": " + location.toHexString());
                Optional<Bytes> node =
              ...
              storageRoot -> {
                try {
                  final StoredMerklePatriciaTrie<Bytes, Bytes> storageTrie =
                      new StoredMerklePatriciaTrie<>(
                          (location, hash) -> {
                            LOG.info("[PRELOAD STORAGE] - " + System.nanoTime() + ": " + location.toHexString());
                            Optional<Bytes> node =
  ...
  public Optional<Bytes> getAccountStateTrieNode(
    ...
    if (nodeHash.equals(MerkleTrie.EMPTY_TRIE_NODE_HASH)) {
      LOG.info("[READ ACCOUNT - EMPTY] - " + System.nanoTime() + ": " + location.toHexString());
      return Optional.of(MerkleTrie.EMPTY_TRIE_NODE);
    } else {
      final Bytes cached = accountNodes.getIfPresent(nodeHash);
      if (cached != null) {
        LOG.info("[READ ACCOUNT - HIT] - " + System.nanoTime() + ": " + location.toHexString());
        return Optional.of(cached);
      } else {
        LOG.info("[READ ACCOUNT - MISS] - " + System.nanoTime() + ": " + location.toHexString());
  ...
  public Optional<Bytes> getAccountStorageTrieNode(
      ...
      final Bytes32 nodeHash) {
    if (nodeHash.equals(MerkleTrie.EMPTY_TRIE_NODE_HASH)) {
      LOG.info("[READ STORAGE - EMPTY] - " + System.nanoTime() + ": " + Bytes.concatenate(accountHash, location).toHexString());
      return Optional.of(MerkleTrie.EMPTY_TRIE_NODE);
    } else {
      final Bytes cached = storageNodes.getIfPresent(nodeHash);
      if (cached != null) {
        LOG.info("[READ STORAGE - HIT] - " + System.nanoTime() + ": " + Bytes.concatenate(accountHash, location).toHexString());
        return Optional.of(cached);
      } else {
        LOG.info("[READ STORAGE - MISS] - " + System.nanoTime() + ": " + Bytes.concatenate(accountHash, location).toHexString());
